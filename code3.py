...
def G2_Net():
    ...
    conv2 = tf.layers.conv2d(inputs = b_norm1, filters = 64, kernel_size = 3, strides = 1, padding = "same", activation = tf.nn.relu, name = "conv2")
    ...
    conv3 = tf.layers.conv2d(inputs = drop1, filters = 128, kernel_size = 3, strides = 1, padding = "same", activation = tf.nn.relu, name = "conv3")
    ...
    conv4 = tf.layers.conv2d(inputs = b_norm2, filters = 128, kernel_size = 3, strides = 1, padding = "same", activation = tf.nn.relu, name = "conv4")
    ...
    pool1_flat = tf.reshape(drop2, [-1, 8 * 8* 128])
    ...
    dense2 = tf.layers.dense(inputs = drop3, units = 512, activation = tf.nn.relu, name = "dense2")
    drop4 = tf.layers.dropout(inputs = dense2,  rate = drop_2, noise_shape = None, seed = None, name = "dropout4")
